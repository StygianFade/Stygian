...
if not exist "%CLANG%" (
  echo [calendar_mini] ERROR: clang not found at "%CLANG%"
  exit /b 1
)

if not exist shaders\build\stygian.vert.glsl (
  echo [calendar_mini] Missing shader outputs. Run build_shaders.bat first.
  exit /b 1
)
...
"%CLANG%" -std=c2x -Wall -O0 -g ^
  ...
  -o build/calendar_mini.exe ^
  -luser32 -lgdi32 -ldwmapi -lopengl32 -lz -lzstd -D_CRT_SECURE_NO_WARNINGS

if %ERRORLEVEL% NEQ 0 (
  echo [calendar_mini] Build FAILED
  exit /b 1
)

echo [calendar_mini] Build SUCCESS: build/calendar_mini.exe
exit /b 0
