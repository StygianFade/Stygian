...
if not exist "%CLANG%" (
  echo [perf_pathological_suite] ERROR: clang not found at "%CLANG%"
  exit /b 1
)

if not exist shaders\build\stygian.vert.glsl (
  echo [perf_pathological_suite] Missing shader outputs. Run build_shaders.bat first.
  exit /b 1
)
...
echo [perf_pathological_suite] Building OpenGL suite...
"%CLANG%" -std=c2x -Wall -O0 -g ^
  ...
  -o build/perf_pathological_suite.exe ^
  -luser32 -lgdi32 -ldwmapi -lopengl32 -lz -lzstd -D_CRT_SECURE_NO_WARNINGS

if %ERRORLEVEL% NEQ 0 (
  echo [perf_pathological_suite] Build FAILED
  exit /b 1
)

echo [perf_pathological_suite] Build SUCCESS: build/perf_pathological_suite.exe
exit /b 0
