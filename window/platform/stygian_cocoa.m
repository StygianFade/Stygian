...
void stygian_window_get_titlebar_hints(StygianWindow *win,
  ...
  if (!out_hints)
    return;
  out_hints->button_order = STYGIAN_TITLEBAR_BUTTONS_LEFT;
  out_hints->supports_hover_menu = false;
  out_hints->supports_snap_actions = false;
  out_hints->recommended_titlebar_height = 32.0f;
  out_hints->recommended_button_width = 14.0f;
  out_hints->recommended_button_height = 14.0f;
  out_hints->recommended_button_gap = 8.0f;
}
