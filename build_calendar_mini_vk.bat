...
if not exist "%CLANG%" (
  echo [calendar_mini_vk] ERROR: clang not found at "%CLANG%"
  exit /b 1
)

if not exist shaders\build\stygian.vert.glsl (
  echo [calendar_mini_vk] Missing shader outputs. Run build_shaders.bat first.
  exit /b 1
)
...
echo [calendar_mini_vk] Building...
"%CLANG%" -std=c2x -Wall -O0 -g ^
  ...
  -o build/calendar_mini_vk.exe ^
  -L"%VULKAN_SDK%\Lib" ^
  -lvulkan-1 -luser32 -lgdi32 -ldwmapi -lopengl32 -lz -lzstd ^
  -D_CRT_SECURE_NO_WARNINGS

if %ERRORLEVEL% NEQ 0 (
  echo [calendar_mini_vk] Build FAILED
  exit /b 1
)

echo [calendar_mini_vk] Build SUCCESS: build/calendar_mini_vk.exe
exit /b 0
